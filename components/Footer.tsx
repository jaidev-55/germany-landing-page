        ...
        <div className="bg-linear-to-r from-blue-600 to-blue-500 rounded-2xl sm:rounded-3xl px-6 sm:px-10 md:px-14 py-8 sm:py-10 flex flex-col md:flex-row items-center justify-between gap-5 sm:gap-6 shadow-[0_8px_40px_-8px_rgba(37,99,235,0.3)]">
          <div>
            ...
            <p className="text-blue-100/70 text-xs sm:text-sm text-center md:text-left">
              Get free expert counseling — from university selection to landing
              in Germany
            </p>
          </div>
          <a
            href="#contact"
            className="group shrink-0 inline-flex items-center gap-2.5 px-6 sm:px-7 py-3 sm:py-3.5 rounded-xl font-bold text-sm sm:text-[0.95rem] text-blue-600 bg-white shadow-[0_4px_16px_-4px_rgba(0,0,0,0.15)] hover:shadow-[0_8px_24px_-4px_rgba(0,0,0,0.2)] hover:-translate-y-px active:translate-y-0 transition-all duration-300"
          >
            Get Free Counseling
            ...
            {socialLinks.map((social: SocialLink) => {
              ...
              return (
                <a
                  key={social.label}
                  href={social.href}
                  className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-white/5 border border-white/6 flex items-center justify-center ${social.hoverColor} hover:border-transparent transition-all duration-200`}
                  aria-label={social.label}
                >
                  <SocialIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400" />
                </a>
              );
            })}
          </div>
